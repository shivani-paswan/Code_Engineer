...
class _RegisterNowState extends State<RegisterNow> {
  ...
  final confirmpasswordController=TextEditingController(); // controller of paasword
FirebaseAuth _auth=FirebaseAuth.instance;      // creating the instance of authentication
  @override
  void dispose() {
        ...
        child: SingleChildScrollView(
          child: Column(
            children: [
              ...
              Text("Create new account",style: TextStyle(color: Colors.white,fontSize: 18,fontWeight: FontWeight.bold),),
              SizedBox(height: 10,),
              Text("Ｐｌｅａｓｅ   ｆｉｌｌ   ｔｈｅ   ｆｏｒｍ  ",style: TextStyle(color: Colors.white,fontSize: 12),),
              SizedBox(height: 5,),

              Text("  ｔｏ   ｃｏｎｔｉｎｕｅ",style: TextStyle(color: Colors.white,fontSize: 12),),
              SizedBox(height: 40,),
              ...
              GestureDetector(
                ...
                onTap: (){
                  if(_formkey.currentState!.validate()){
                    setState((){    // for loading on in   btn
                     loading=true;
                    });
                    _auth.createUserWithEmailAndPassword(            // firebase work of register data
                        email: userController.text.toString(),
                        password: passwordController.text.toString()).then((value) {
                      setState((){         // for loading off in btn
                        loading=false;
                        userController.clear();          // both statements are valid to clean textfeild after register
                      ...
                      debugPrint(error.toString());  // for handle of th e debug statements
                ...
                child: Container(
                  ...
                  child:loading?CircularProgressIndicator(): Text('Register',style: TextStyle(color: Colors.white,fontSize: 17),),
              ...
              GestureDetector(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    Text(" Login Now",style: TextStyle(color: Colors.grey,fontSize: 17),),
                  ],
                ),
              ),
              ...
              Row(
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  Text("Login with",style: TextStyle(color: Colors.white,fontSize: 17),),
                  InkWell(child: Text(" Phone Number",style: TextStyle(color: Colors.grey,fontSize: 17),)),
                ],
              ),
